...
namespace Checkers
{
    ...
    //будут недействительны во время турнира
    {
        ...
        {
            Color = color;
            var enemyColor=Color.White;
            if (enemyColor==color)
                enemyColor=Color.Black;
            MyEnemy = new Enemy(enemyColor);
        }
        Color Color;
        Enemy MyEnemy;
        ...
        public int EvaluateMove(Checker[,] field, Move move)
        {
            ...
            var bestEnemyMove = MyEnemy.GetEvaluatedMoves(tempMap, MyEnemy.Color)
                                     .OrderByDescending(x => x.Value)
                                     .First()
                                     .Key;
            valid.MakeMove(tempMap, bestEnemyMove);
        ...
        class Enemy
        {
            public Dictionary<Move, int> GetEvaluatedMoves(Checker[,] field, Color color)
             {
                var valid = new Validator();
                var dict = new Dictionary<Move, int>();
                foreach (var move in valid.GetBindingMoves(field, color))
                ...
                return dictOfEvaluatedPossibleMoves;
            }
            ...
            public Color Color;
            int EvaluateMove(Checker[,] field, Move move)
            {
                var tempMap = new Checker[8, 8];
